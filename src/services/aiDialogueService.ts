...
function buildGameStateContext(gameState: GameState) {
  ...
  return {
    ...
    currentDate,
    currentLocation: player.homeBase || player.currentTeam.name,
    currentCountry: player.currentCountry,
    schoolStage: player.schoolStage,
    isPro: player.age >= 18 || player.schoolStage === 'pro' || player.wage > 0,
    funds: player.funds ?? 30000,
